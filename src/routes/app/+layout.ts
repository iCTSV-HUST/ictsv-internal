...
const navList: Route[] = [
	...
	},
	{
		...
		permission: ({ rank, depts }) =>
			depts.includes('Tiểu ban') || rank === Rank.TOPHO || rank === Rank.TOTRUONG,
		iconName: 'attendance'
	},
	{
...
async function tryRefreshToken(fetch: {
	...
	try {
		...
		if (currentUser.tryRefresh() === false) {
			currentUser.setInfoFromRecord(
				await pb.collection('members').getOne(pb.authStore.model?.id, {
